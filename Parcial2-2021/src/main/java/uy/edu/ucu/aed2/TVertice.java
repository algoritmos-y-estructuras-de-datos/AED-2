...
public class TVertice<T> implements IVertice {
    ...
    public TVertice rutaMenosSaltos(Comparable destino) {
        Deque<TVertice> porVisitar = new LinkedList<>();

        porVisitar.push(this);
        this.setVisitado(true);
        if (this.getEtiqueta().compareTo(destino) == 0) {
        ...
        while (!porVisitar.isEmpty()) {
            ...
            for (Object a : v.getAdyacentes()) {
                ...
                TVertice vDestino = ad.getDestino();
                if (!vDestino.getVisitado()) {
                    porVisitar.add(vDestino);
                    vDestino.setVisitado(true);
                    vDestino.setPredecesor(v);
                    if (vDestino.getEtiqueta().compareTo(destino) == 0) {
                        return vDestino;
                    }
                }
            }
