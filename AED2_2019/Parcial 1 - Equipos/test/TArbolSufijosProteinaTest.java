...
public class TArbolSufijosProteinaTest {
    ...
    public void testEncontrarPatron_encuentroDosSufijos() {
        ...
        assertEquals(arbolCadena.encontrarPatron("ST").get(1), new Integer(2));
    }
     ...
     */
    @Test
    public void testEncontrarPatron_encuentroAllSufijos() {
        assertFalse(arbolCadena.encontrarPatron("T").isEmpty());
        assertEquals(arbolCadena.encontrarPatron("T").get(0), new Integer(6));
        assertEquals(arbolCadena.encontrarPatron("T").get(1), new Integer(4));
        assertEquals(arbolCadena.encontrarPatron("T").get(2), new Integer(0));
        assertEquals(arbolCadena.encontrarPatron("T").size(), 3);
        assertEquals(arbolCadena.encontrarPatron("STST").get(0), new Integer(2));
        assertEquals(arbolCadena.encontrarPatron("STST").size(), 1);
        assertEquals(arbolCadena.encontrarPatron("ESTST").get(0), new Integer(1));
        assertEquals(arbolCadena.encontrarPatron("ST").get(0), new Integer(5));
        assertEquals(arbolCadena.encontrarPatron("ST").get(1), new Integer(2));
        assertEquals(arbolCadena.encontrarPatron("ST").size(), 2);
    }
