...
public class Ordenador_SeleccionDirecta {
    
    public static TProducto[] ordenarPorSeleccion(TProducto[] datosParaClasificar, boolean ascendente) {
        for (int i = 0; i < datosParaClasificar.length - 1; i++) {
            int indice_Menor_Mayor = i;
            long claveMenor = datosParaClasificar[i].getCodigo();
            for (int j = i + 1; j < datosParaClasificar.length; j++) {
                if (ascendente){
                    if (datosParaClasificar[j].getCodigo() < claveMenor) {
                        indice_Menor_Mayor = j;
                        claveMenor = datosParaClasificar[j].getCodigo();
                    }
                }else{
                    if (datosParaClasificar[j].getCodigo() > claveMenor) {
                        indice_Menor_Mayor = j;
            ...
            intercambiar(datosParaClasificar, i, indice_Menor_Mayor);
        }
...
//        System.out.println("TAMAÑO LISTA="+lista.size());
        ...
        TProducto[] listaAsc = ordenarPorSeleccion(productos,ordenAscendente);
...
//        }
        ...
        TProducto[] listaDesc = ordenarPorSeleccion(productos,ordenAscendente);
