...
public class Ordenador_CuentaPorDistribucion {
    
    private static TProducto getMin(TProducto[] entrada){
        long min = Long.MAX_VALUE;
        TProducto menor = null;
        for(int j=0;j<entrada.length;j++){
            if (entrada[j].getCodigo()<min){
                min = entrada[j].getCodigo();
                menor = entrada[j];
            }
        }
        return menor;
    ...
    public static TProducto[] ordenarPorCuentaPorDistrubucion(TProducto[] datosParaClasificar, boolean ascendente){
        TProducto u = getMin(datosParaClasificar);
        TProducto v = getMax(datosParaClasificar);
        //System.out.println("Min: "+u + " - Max: "+v);
        
        TProducto[] R = copiar(datosParaClasificar);
        TProducto[] S = new TProducto[R.length];
        int N = R.length;
        if (v!=null && u!=null){
            ...
            int lugares = (int) (N / v.getCodigo());
            int[] cuenta = new int[lugares];
            ...
            for(int j=0; j<R.length; j++){
                int nroRelativo =  (int) (N / R[j].getCodigo());
                cuenta[nroRelativo] += 1;
            }
            
            if(ascendente){
//                //Cuenta la distribución final de los elementos
//                for(long i=1; i<=v.getCodigo(); i++){
...
//        System.out.println("TAMAÑO LISTA="+lista.size());
        ...
        TProducto[] listaAsc = ordenarPorCuentaPorDistrubucion(productos,ordenAscendente);
...
//        }
        ...
        TProducto[] listaDesc = ordenarPorCuentaPorDistrubucion(productos,ordenAscendente);
