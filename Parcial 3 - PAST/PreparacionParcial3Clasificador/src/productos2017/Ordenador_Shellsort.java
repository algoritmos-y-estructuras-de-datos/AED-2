...
public class Ordenador_Shellsort {
    
    public static TProducto[] ordenarPorShell(TProducto[] datosParaClasificar, boolean ascendente) {
        int j, inc;
        int[] incrementos = new int[] { 3223, 358, 51, 10, 3, 1 };
        for (int posIncrementoActual = 1; posIncrementoActual < incrementos.length; posIncrementoActual++) {
            inc = incrementos[posIncrementoActual]; //k
            if (inc < (datosParaClasificar.length / 2)) {
                for (int i = inc; i < datosParaClasificar.length; i++) {
                    TProducto aux = datosParaClasificar[i];
                    j = i - inc;
                    if (ascendente){
                        while (j >= 0 && aux.getCodigo() < datosParaClasificar[j].getCodigo()) {
                            if (datosParaClasificar[j].getCodigo() > datosParaClasificar[j + inc].getCodigo()) {
...
//        System.out.println("TAMAÑO LISTA="+lista.size());
        ...
        TProducto[] listaAsc = ordenarPorShell(productos,ordenAscendente);
...
//        }
        ...
        TProducto[] listaDesc = ordenarPorShell(productos,ordenAscendente);
