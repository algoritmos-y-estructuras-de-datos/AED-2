...
public class Ordenador_InsercionDirecta {
    
    public static TProducto[] ordenarPorInsercion(TProducto[] datosParaClasificar, boolean ascendente) {
        if (datosParaClasificar != null) {
            for (int i = 1; i < datosParaClasificar.length; i++) {
                TProducto aux = datosParaClasificar[i];
                int j = i - 1;
                if (ascendente){
                    while ((j >= 0) && (aux.getCodigo() < datosParaClasificar[j].getCodigo())) { //Cambia el simbolo a mayor "aux > datosParaClasificar[j]"
...
//        System.out.println("TAMAÑO LISTA="+lista.size());
        ...
        TProducto[] listaAsc = ordenarPorInsercion(productos,ordenAscendente);
...
//        }
        ...
        TProducto[] listaDesc = ordenarPorInsercion(productos,ordenAscendente);
