...
public class GeneradorDatosGenericos {
	private static int TAMANIO_MAX = Integer.MAX_VALUE;
	...
	public int[] generarDatosDescendentes() {
        ...
        public int[] generarDatosAleatorios(int cant) {
		Random rnd = new Random();
		int[] datosGenerados = new int[cant];
		boolean[] datosUtilizados = new boolean[cant];
		for (int i = 0; i < datosGenerados.length; i++) {
			int j = rnd.nextInt(cant);
			while(datosUtilizados[j]){
				j = (j + 1) % cant;
			}
			datosGenerados[j] = i;
	...
	public int[] generarDatosAscendentes(int cant) {
		int [] copiaAscendente = new int[cant];
        ...
        public int[] generarDatosDescendentes(int cant) {
		int [] copiaDescendente = new int[cant];
		for (int i = 0; i < cant; i++) {
			copiaDescendente[i] = cant - i;
		}
		return copiaDescendente;
	}
}
